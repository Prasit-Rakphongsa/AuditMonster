...
// P'momo
    ...
    foreach($event_field_relation as $key => $value){
        $filtered_event_field_relation[$value["event_code"]][$value["sub_event_code"]][$value["field_name"]] = array($value["generator_value"],$value["send_to_edw"]);
    }
    // print_r($filtered_event_field_relation);
    ...
    for($i=1; $i<count($event); $i++){
        if($event[$i]["send_to_edw"] == "Y" or $event[$i]["send_to_edw"] == "y"){
            ...
            for($k=2; $k<count($field); $k++){
                if($field[$k]["field_name"] == "SUB_EVENT_CODE"){
                ...
                else if(array_key_exists($field[$k]["field_name"],$filtered_event_field_relation[$event[$i]["event_code"]][$event[$i]["sub_event_code"]])){
                    ...
                    // } else 
                    if($field[$k]["send_to_edw"] == "Y" or $field[$k]["send_to_edw"] == "y"){ // check field whether send to edw
                        if($filtered_event_field_relation[$event[$i]["event_code"]][$event[$i]["sub_event_code"]][$field[$k]["field_name"]][1] == "Y" or $filtered_event_field_relation[$event[$i]["event_code"]][$event[$i]["sub_event_code"]][$field[$k]["field_name"]][1] == "y"){ // check relation whether send to edw
                            // echo $event[$i]["event_code"]." ".$event[$i]["sub_event_code"]." ".$event[$i]["event_name_eng"]." ";
                            // echo $field[$k]["field_name"]." ";
                            // print_r($filtered_event_field_relation[$event[$i]["event_code"]][$event[$i]["sub_event_code"]][$field[$k]["field_name"]]);
                            // echo "</br>";
                            if($field[$k]["field_name"] == "SUB_EVENT_CODE"){
                            
                            } else if($field[$k]["field_name"] == "INFORMATION_1"){
                                array_push($row,substr($filtered_event_field_relation[$event[$i]["event_code"]][$event[$i]["sub_event_code"]][$field[$k]["field_name"]][0], 1, -1));
                                ...
                                array_push($row,substr($filtered_event_field_relation[$event[$i]["event_code"]][$event[$i]["sub_event_code"]][$field[$k]["field_name"]][0], 1, -1));
                            } else if($field[$k]["field_name"] == "INFORMATION_6"){
                                continue;
                            } else{
                ...
                // for the fields that are not in the event, then fill blank.
                    ...
                    if($field[$k]["field_name"] != "INFORMATION_6"){
                        array_push($row,NULL);   
                    }
                }
